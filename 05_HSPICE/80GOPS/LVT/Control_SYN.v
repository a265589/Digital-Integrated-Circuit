...
module Control ( clk, rst_n, out_valid );
  ...
  ASYNC_DFFHx1_ASAP7_75t_R out_valid_reg ( .D(N12), .CLK(clk), .RESET(n4), 
        .SET(n11), .QN(n3) );
  NAND2xp33_ASAP7_75t_R U12 ( .A(n7), .B(n5), .Y(n9) );
  AND2x2_ASAP7_75t_R U13 ( .A(n7), .B(n5), .Y(n14) );
  BUFx5_ASAP7_75t_R U14 ( .A(n6), .Y(n10) );
  INVx3_ASAP7_75t_R U15 ( .A(n10), .Y(n16) );
  INVx6_ASAP7_75t_R U16 ( .A(rst_n), .Y(n11) );
  INVx6_ASAP7_75t_R U17 ( .A(rst_n), .Y(n12) );
  INVx6_ASAP7_75t_R U18 ( .A(rst_n), .Y(n13) );
  INVx6_ASAP7_75t_R U19 ( .A(rst_n), .Y(n8) );
